...
ds_IMI2_FCDi <- merge(ds_IMI2_QFGDi, ds_IMI2_QDP, by = "cod_programa", all.x = T) %>% 
  ...
  filter(DFGDi <= 1) %>% ## Limpeza de ocorrências acima de 1.0
...
ds_IPT %<>% mutate(QPPa = SPPP/QPPP,
  ...
  filter(QPPa <= 1, QPPr <= 1, QPA <= 1) %>% ## Limpeza de ocorrências acima de 1.0
...
GGCor <- ggplot(data = ds, aes(x = ds[,1])) + 
  ...
  geom_label(aes(x = 0.5, y = 32, label = 'Correlação média ^'), 
             col = 'blue', hjust = 'left', alpha = 0.8) + 
  geom_hline(yintercept = 60, col = 'green', alpha = 0.9) + 
  geom_label(aes(x = 0.5, y = 62, label = 'Correlação alta ^'), 
             col = 'green', hjust = 'left', alpha = 0.8) + 
