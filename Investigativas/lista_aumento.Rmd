...
#### Processamento de prestadores com acentuado aumento de fatura

```{r main, fig.height=7, fig.width=10, message=FALSE, warning=FALSE}
...
f.plot <- function(df) {
  ...
  gg <- ggplot(data = df , aes(x = anomes)) +
    ...
    labs(title = paste0('Faturamento do Prestador: ', df[1,2], ' - ', df[1,1]),
         x = 'Ano e mês', 
         y = 'Valores') +
    theme_gray() +
...
```{r plot, fig.height=7, fig.width=10, message=FALSE, warning=FALSE}
...
df_prestadores_att %<>% arrange(desc(sum), nome, codigo)

kable(select(df_prestadores_att, codigo, nome, sum, avg, tav_hav, mx_mi, av_mi, mx_av), 
      caption = 'Prestadores marcados')
