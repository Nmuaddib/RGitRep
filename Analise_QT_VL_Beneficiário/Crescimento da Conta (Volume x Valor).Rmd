...
```{sql connection=con, output.var="x_vv_main"}
...
# ------------------------------------------------------------------------------------------
# x_vv_main_ip <- read_excel('VV_extract_sqlbi.xls')
...
```{sql connection=con, output.var="x_vv_faixa"}
select  to_char(pcm.mes_ano_ref,'RRRRMM') anomes,
        fe.DESC_FAIXA faixa,
        sum(qtd_ativos) BT
from    ts.posicao_cadastro_mes pcm,
...
x_vv_faixa_ip <- read_excel('beneficiarios_faixa.xlsx')
...
x_vv_faixa %<>% 
  mutate(ANO = factor(str_sub(ANOMES, 1, 4)), 
         ANOMES = factor(ANOMES), 
         FAIXA = factor(FAIXA, levels = c("ate 18 anos", "19 a 23 anos", 
                                          "24 a 28 anos", "29 a 33 anos", 
                                          "34 a 38 anos", "39 a 43 anos", 
                                          "44 a 48 anos", "49 a 53 anos", 
                                          "54 a 58 anos", "59 anos ou mais"))) %>% 
