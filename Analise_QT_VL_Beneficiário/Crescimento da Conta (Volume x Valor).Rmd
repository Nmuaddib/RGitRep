...
x_vv_main_c$COMPONENTE[is.na(x_vv_main_c$COMPONENTE)] <- 'NA'

#x_vv_classe <- read_excel('VV_componente_class.xlsx')
x_vv_classe <- read_excel('VV_componente_comp.xlsx')
x_vv_classe$COMPONENTE[is.na(x_vv_classe$COMPONENTE)] <- 'NA'

x_vv_main %<>% 
  mutate(ANOMES = factor(.$ANOMES), ANO = factor(.$ANO)) %>% 
  as.tibble()

x_vv_main_c %<>% merge(., x_vv_classe, by = c('INTERNACAO','COMPONENTE')) %>% 
  filter(CLASSE != 'NA') %>%   
  mutate(ANOMES = factor(.$ANOMES), 
         ANO = factor(.$ANO), 
         CLASSE = factor(paste0(.$ANO,'-', .$CLASSE))) %>% 
  as.tibble()
```
