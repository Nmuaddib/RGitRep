...
##Processamento detalhado por prestador

Para separação dos seguintes grupos de indicadores:
...
f.gg_param <- function(nm) {
  ...
  ggp <- ggplot(data = FA.lst_prestador[[nm]], aes(x = ANOMES, y = escala_geral)) +
    ...
    geom_label(aes(x = 1, y = 5.3, label = '% Acumulado de Meses Anteriores'), 
...
- A parte superior (entre 4 e 5 no eixo vertical, representando 0% a 100%) apresenta linha da variação percentual para o valor faturado, que é referente à outros meses de atendimento. Quanto mais próximo da linha verde, maior o faturamento de uma referência que foi atendido naquela mesma referência. Se a linha azul está sempre próxima à vermelha, o prestador tem a maior parcela de cada faturamento referente à meses anteriores. A linha acendente demonstra um período de acumulo de contas.
