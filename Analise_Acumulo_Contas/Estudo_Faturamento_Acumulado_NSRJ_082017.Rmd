...
x_geral <- cbind(x_geral, ID_AM = 1:43)
...
f.tb_param <- function(nm, i) {
  ...
  tbp <- FA.lst_prestador[[nm]] %>% 
    mutate(VA_FT = round(VA_FT), 
           VA_AT = round(VA_AT),
           P_FCR = round((P_FCR*100), digits = 2)) %>% 
...
f.gg_da <- function(nm) {
  ...
  ggp <- ggplot(data = FA.lst_prestador[[nm]], aes(x = ANOMES)) +
    geom_line(aes(y = (VA_FT)/1000), 
              size = 0.9, col = 'dark green', alpha = 1, group = 1) +
    geom_line(aes(y = (VA_AT)/1000), 
